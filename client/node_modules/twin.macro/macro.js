...
var logBadGood = function (bad, good) { return spaced(((color.error('✕ Bad:')) + " " + bad + "\n" + (color.success('✓ Good:')) + " " + good)); };
...
var errorSuggestions = function (properties) {
  ...
  if (isCsOnly) { return spaced(((color.highlight(className)) + " isn’t in the right syntax for use in the cs prop\n\nTry something like maxWidth[100vw]\nRead more at https://twinredirect.page.link/cs-classes")); }
...
var addDataTwPropToPath = function (ref) {
  ...
  dataProperty.forEach(function (path$$1) { return path$$1.remove(); }); // Replace the "stand-in spaces" with real ones

  var originalClasses = rawClasses.replace(new RegExp(SPACE_ID, 'g'), ' '); // Add the attribute

  path$$1.insertAfter(t.jsxAttribute(t.jsxIdentifier(propName), t.stringLiteral(originalClasses)));
};
...
var addDataPropToExistingPath = function (ref) {
  ...
  var originalClasses = rawClasses.replace(new RegExp(SPACE_ID, 'g'), ' '); // Add a new attribute

  path$$1.pushContainer('attributes', t.jSXAttribute(t.jSXIdentifier(propName), t.jSXExpressionContainer(t.stringLiteral(originalClasses))));
};
...
var getProperties = function (className, state, ref) {
  ...
  var type = isStatic && 'static' || isDynamicClass && 'dynamic' || corePlugin && 'corePlugin' || isCss && 'css';
  return {
    type: type,
...
function spreadVariantGroups(classes, context, importantContext, start, end) {
  ...
  while (match = reg.exec(classes)) {
    ...
    if (variant) {
      ...
      var cssClass = classes.slice(classes.indexOf(className), closeBracket$1 + 1);
      var spaceReplacedClass = cssClass.replace(/ /g, SPACE_ID); // Convert spaces in classes to a temporary string so the css won't be
...
var handleCss = (function (ref) {
  ...
  var className = ref.className;
  var ref$1 = className.replace(']', '') // Replace the "stand-in spaces" with real ones
  .replace(new RegExp(SPACE_ID, 'g'), ' ').split('[');
...
var twinMacro = function (ref) {
  ...
  program.traverse({
    ...
    JSXElement: function JSXElement(path$$1) {
      ...
      var index = ref.index;
      var hasCssAttribute = ref.hasCssAttribute;
      state.hasCssAttribute = hasCssAttribute; // Reverse the attributes so the items keep their order when replaced
